...
function activate(context) {
  ...
  setInterval(async () => {
    try {
      const res = await axios_default.get(`${HUB_URL}/check-save`);
  ...
  async function captureAndSubmit() {
    const tabs = vscode.window.tabGroups.all.flatMap((g) => g.tabs).filter((t) => t.input instanceof vscode.TabInputText).map((t) => t.input.uri.fsPath);
    const payload = {
      type: "vscode",
      // So the Hub knows where to put it in the JSON
      data: {
        openFiles: tabs,
        activeFile: vscode.window.activeTextEditor?.document.fileName
      }
    };
    await axios_default.post(`${HUB_URL}/snapshot`, payload);
    vscode.window.setStatusBarMessage("\u2705 Flow: Auto-Saved!", 3e3);
  }
}
